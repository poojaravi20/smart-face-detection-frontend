...
class App extends Component {
  ...
  calculateFaceLocation = (data) => {
    ...
    const facesArray = data.outputs[0].data.regions.map((clarifaiFace) => {
      return {
  ...
  onButtonSubmit = () => {
    this.setState({imageUrl: this.state.input});
      fetch('https://safe-mesa-77780.herokuapp.com/imageurl', {
        method: 'post',
      ...
      .then(response => {
        if (response) {
          fetch('https://safe-mesa-77780.herokuapp.com/image', {
            method: 'put',
            headers: {'Content-Type': 'application/json'},
            body: JSON.stringify({
